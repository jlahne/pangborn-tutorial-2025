...
```{r setup-3, echo=FALSE, purl = FALSE, include=FALSE}
library(tidyverse)
...
In the last several plots, notice that we are using the default (and to my mind unattractive) grey background of `ggplot2`.  If this tutorial had a single goal, it would be to eliminate this theme from published papers.  It looks *bad*.

```{r adding a different theme}
ca_cider_coords %>%
  ggplot(mapping = aes(x = `Dim 1`, y = `Dim 2`)) + 
  geom_point(mapping = aes(color = type)) +
  ggrepel::geom_text_repel(mapping = aes(label = name),
...
```{r setting the default theme then making a ggplot}
theme_set(
  theme_bw() + 
